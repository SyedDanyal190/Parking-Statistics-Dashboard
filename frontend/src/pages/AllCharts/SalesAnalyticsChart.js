...
const SalesAnalyticsChart = ({ trafficData }) => {
  ...
  useEffect(() => {
    if (
      ...
      Object.keys(trafficData.resultSameDate).length > 0
    ) {
    ...
    //     },
    {
      ...
      data: isSingleDay
        ...
        : dateKeys.map((date) => {
            // Get the pre-calculated daily average duration from the backend
            const dailyAverage =
              trafficData.resultNew[date]?.dailyAverageDuration;
        ...
        custom: function ({ series, seriesIndex, dataPointIndex, w }) {
          ...
          if (isSingleDay) {
                ...
                if (!vehicleDataAdded) {
                  ...
                  vehicleData.forEach((vehicle) => {
                    if (
                      ...
                      let formattedVehicleDuration = formatDuration(
                        vehicle.avgDuration
                      );
                      tooltipHtml += `
                                  ...
                                  } : ${formattedVehicleDuration}</span><br/>
                              `;
                      shownVehicles.add(
                        vehicle.type + vehicleTypeIndices[vehicle.type]
                      );
                  ...
                  tooltipHtml += `
                          <span style="color: ${
                            colorMapping.totalCost
                          }; font-size: 18px; line-height: 18px;">●</span>  
                          <span style="color: black">Total Cost: $${totalCost.toFixed(
                            2
                          )}</span><br/>
                      `;
            ...
            tooltipHtml += `
              ...
              <span style="color: black">Total Cost: $${totalCostSum.toFixed(
                2
              )}</span><br/>
          `;
          }
